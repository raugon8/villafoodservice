...
from object_class.ingredient import IngredienteResponse, IngredienteCreate, IngredienteUpdate
# Se asume que Adán creará este archivo de servicios
from services import ingrediente_service
from database_manager.db_setup import get_db

router = APIRouter(prefix="/ingredientes", tags=["ingredientes"]) [cite: 49]

@router.get("/", response_model=List[IngredienteResponse]) [cite: 49]
def listar_ingredientes(db: Session = Depends(get_db)):
    return ingrediente_service.get_ingredientes(db)

@router.get("/{id}", response_model=IngredienteResponse) [cite: 49]
...
@router.post("/", response_model=IngredienteResponse, status_code=201) [cite: 49]
...
@router.put("/{id}", response_model=IngredienteResponse) [cite: 49]
...
@router.delete("/{id}", status_code=204) [cite: 50, 54]
